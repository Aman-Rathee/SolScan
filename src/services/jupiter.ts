...
export const TOKEN_INFO: Record<string, { symbol: string; name: string; decimals: number; color: string }> = {
...
export async function getSwapQuote(
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            console.error(`[jupiter] attempt ${attempt} failed:`, lastError.message);
            if (attempt < 3) {
